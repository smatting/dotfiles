...
-- Migrated to nvim-treesitter v1.0 API:
-- highlight and indent are now neovim built-ins (auto-enabled when parsers are present)
-- incremental_selection removed upstream; textobjects moved to their own plugin setup
vim.defer_fn(function()
  -- Ensure parsers are installed (idempotent)
  require('nvim-treesitter.install').install({
    'c', 'cpp', 'go', 'lua', 'python', 'rust', 'tsx', 'javascript', 'typescript', 'vimdoc', 'vim', 'bash', 'purescript',
  })

  -- Textobjects are now configured via their own plugin
...
--  This function gets run when an a buffer connects
